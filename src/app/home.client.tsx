            ...
            <div className="p-5 sm:p-7">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-stretch mb-6">
                <div className="flex w-full items-center rounded-2xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/70 p-1.5 shadow-sm">
                  <button
                    ...
                    aria-label="Translate text to Morse code"
                     className={`flex flex-1 items-center justify-center gap-2 rounded-xl px-4 py-3 text-sm font-bold transition-all ${
                      mode === "text-to-morse"
                        ? "bg-green-700 text-white shadow-md shadow-green-700/20"
                        : "text-slate-700 hover:bg-white hover:text-slate-900 dark:text-slate-300 dark:hover:bg-slate-700 dark:hover:text-white"
                    }`}
                  >
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
                <div className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-slate-50/70 dark:bg-slate-800/40 p-4 sm:p-5">
                  ...
                  <textarea id="morse-input" aria-label={mode === "text-to-morse" ? "Enter text to translate to Morse code" : "Enter Morse code to decode into text"} value={mode === "text-to-morse" ? text : morseInput} onChange={(e) => mode === "text-to-morse" ? setText(e.target.value) : setMorseInput(e.target.value)} placeholder={mode === "text-to-morse" ? "Type your message here..." : "Enter dots and dashes..."} className="w-full h-[150px] resize-none rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-950 px-4 py-3.5 font-mono text-base text-slate-800 dark:text-slate-100 placeholder:text-slate-500 outline-none transition-all focus:border-green-500 focus:ring-4 focus:ring-green-500/10" />
                </div>

                <div className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-slate-50/70 dark:bg-slate-800/40 p-4 sm:p-5">
                  <div className="flex items-center justify-between mb-3"><label htmlFor="morse-output" className="text-sm font-bold text-slate-700 dark:text-slate-200">Output</label><span className="inline-flex items-center gap-1.5 text-xs font-medium text-green-800 dark:text-green-400"><Zap className="w-3.5 h-3.5" /> Live</span></div>
                  <div id="morse-output" role="status" aria-live="polite" aria-label="Morse code translation output"
...
<div>
  ...
  <input
    id="morse-frequency"
    aria-label="Morse code audio frequency in hertz"
    type="range"
    min="300"
    max="1000"
    step="50"
...
<div>
  ...
  <input
    id="morse-volume"
    aria-label="Morse code audio volume"
    type="range"
    min="0"
    max="1"
    step="0.1"
